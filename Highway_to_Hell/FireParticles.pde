...
class FireParticles {
  ...
  void emit(int particleAmount) {
    ...
    for (int iParticle=0; iParticle<particleAmount; iParticle++) {
      ...
      FireParticle particle = new FireParticle(x, y, framesToLive, particleShape);
  ...
  void update() {
    for (int iParticle=0; iParticle<particles.size(); iParticle++) {
      FireParticle particle = particles.get(iParticle);
      particle.vy += gravity;
      particle.update();
...
class FireParticle {
  ...
  // Constructor
  FireParticle(float x, float y, int framesToLive, String particleShape) {
    this.x = x; 
    this.y = y;
